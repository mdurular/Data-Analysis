...
--1
SELECT DISTINCT B.[YEAR],B.[MONTH], ROUND(CAST ( B.NEXT_CUST AS FLOAT)/A.CURR_CUST,2) AS RETENTION_RATE
...
(SELECT DISTINCT B.[YEAR],
				B.[MONTH],
				B.CURRENT_MONTH,
				ROUND(CAST(B.NEXT_CUST AS FLOAT)/A.CURR_CUST,2) AS RETENTION_RATE
FROM CURRENT_NUM_OF_CUST A INNER JOIN NEXT_NUM_OF_CUST B
